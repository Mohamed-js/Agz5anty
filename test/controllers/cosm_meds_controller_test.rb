...
class CosmMedsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create cosm_med' do
    assert_difference('CosmMed.count') do
      post cosm_meds_url,
           params: { cosm_med: { description: @cosm_med.description, img: @cosm_med.img, main: @cosm_med.main,
                                 name: @cosm_med.name, quantity: @cosm_med.quantity } }
    end
